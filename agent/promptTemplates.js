...
const createArrayFromSingleDocPrompt = (request) => {
  ...
  const parseRequestsPrompt = [
    {
      role: "user",
      content: `You are an AI assistant and expert paralegal assisting trial attorneys in drafting discovery responses.  As part of your job, you convert discovery request documents from text to other formats, such as JSON, for further processesing. I will provide you with some instructions, then a discovery request document, and you will perform a parsing operation.
      The instructions: Review the discovery request document and return the numbered requests, and only the requests, as JSON.  Do not include any of the preamble or earlier sections of the document in your finished result.  This JSON should be in the following form: an array, containing one object for each numbered request.  The object should contain two key/value pairs: “requestId." this key's value will be a randomly generated, version 4 UUID   The second key is ”text".”  Its value will be the substantive textual content of the individual request.  This is an example response to a hypothetical sequence of three requests:  [{ "requestId": "73a855af-4fdc-4b8b-9e7f-34d57a2c084a",
      ...
      "text": "example - request text would be set forth here.“}]. Some documents may contain sections preceding the requests. such as “Definitions”, “Instructions”.  The requests themselves will usually be last.  Parse the text and return the requests only as described above,  do not respond to or answer the requests. Do not add introductory comments, analysis, or observations, such as "Based on  your request, I have parsed the document and..,"  etc.  Such comments will be problematic for later  processing of the data.
      This is the request document: ${request}`,
    },
  ];
